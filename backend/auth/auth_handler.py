...
class AuthHandler:
    ...
    async def get_all_users(self) -> Union[list[User], AuthError]:
        """
        Retrieves a list of all registered users.  This function should 
        only be accessible by super admin users.
        """

        if not self.user or not self.user.is_super_admin:
            return AuthError("Insufficient permissions to list users")

        try:
            self.db.exec("SELECT * FROM users")
            results = self.db.fetchall()
            users = results  # Return public representation
            return users
