...
class APIKeyManager:
    ...
    def issue_new(self, body: APIKeyReq) -> Union[Success, Error]:
        try:
            api_key = f"ekb_{secrets.token_urlsafe(16)}"
            now = datetime.now()
            ...
            self.db.exec(
                ...
                (
                    ...
                    self.user.id,
                    exp,
                    str(body.allowed_origins).replace('[','{').replace(']','}'),  # Serialize if needed
                    str(body.scope).replace('[','{').replace(']','}'), 
                    now,
                ),
            )
            self.db.commit()
    ...
    def get_my_token_list(self) -> dict:
        if self.user:
            self.db.exec(
                "SELECT api_key, name, allowed_origins, scope, expire_ts, last_used, disabled, date_disabled, created_ts FROM api_keys WHERE user_id = %s",
                (self.user.id,),
            )
